...
def calculate_best_players(qSet, identifier):
	...
	normalise_cols = ['goals', 'goals_per_90', 'xG', 'shots', 'shot_percent',
					  'key_passes', 'pass_completion', 'dribbles', 'assists', 'assists_per_90',
					  'int_per_90', 'tackles_per_90', 'tackle_ratio', 'average_rating']
	for col in normalise_cols:
		df[col] = (df[col] - df[col].mean()) / (df[col].std())

	df['defensive'] = (0.1 * df['average_rating'] + 0.3 * df['tackles_per_90'] + 0.15 * df['tackle_ratio'] + df[
		'int_per_90'] * 0.3)
	df['creative'] = (0.1 * df['average_rating'] + 0.2 * df['assists_per_90'] + 0.15 * df['assists'] + df[
		'key_passes'] * 0.3 + df['dribbles'] * 0.2 + df['pass_completion'] * 0.3)
	df['attacking'] = (
				0.1 * df['average_rating'] + df['goals'] * 0.1 + df['goals_per_90'] * 0.32 + df['xG'] * 0.12 + df[
			'shots'] * 0.15 + df['shot_percent'] * 0.1)
...
def calculate_best_players2(qSet, identifier):
	...
	df['defensive'] = (0.1 * df['average_rating'] + 0.3 * df['tackles_per_90'] + 0.15 * df['tackle_ratio'] + df[
		'int_per_90'] * 0.3)
	df['creative'] = (0.1 * df['average_rating'] + 0.2 * df['assists_per_90'] + 0.15 * df['assists'] + df[
		'key_passes_per_90'] * 0.3 + df['dribbles_per_90'] * 0.2 + df['pass_completion'] * 0.3)
	df['attacking'] = (
				0.1 * df['average_rating'] + df['goals'] * 0.1 + df['goals_per_90'] * 0.32 + df['xG'] * 0.12 + df[
			'shots_per_90'] * 0.15 + df['shot_percent'] * 0.1)
