...
def calculate_best_players(qSet, identifier):
  ...
  df['creative'] = (0.1 * df['average_rating'] + 0.2 * df['assists_per_90'] + 0.15 * df['assists'] + df['key_passes'] * 0.3 + df['dribbles'] * 0.2 + df['pass_completion'] * 0.3)
  df['attacking'] = (0.1 * df['average_rating'] + df['goals'] * 0.1 + df['goals_per_90'] * 0.32 + df['xG'] * 0.12 + df['shots'] * 0.15 + df['shot_percent'] * 0.1)
