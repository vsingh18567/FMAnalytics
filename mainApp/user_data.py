...
class UserData:
    ...
    def add_data(self, player: Player, row: pd.Series) -> None:
        ...
        player_season.assists_per_90 = per_90(
            player_season.assists, player_season.minutes
        )
        player_season.save()
        ...
        def per90_calculation(
            ovr_var,
            season_var,
            ovr_min=player.minutes,
            season_min=player_season.minutes,
        ):
            try:
                return (ovr_var * ovr_min + season_var * season_min) / (
                    ovr_min + season_min
                )
            except:
                return 0

        player.goals_per_90 = per90_calculation(
            player.goals_per_90, player_season.goals_per_90
        )
        player.assists_per_90 = per90_calculation(
            player.assists_per_90, player_season.assists_per_90
        )
        player.tackles_per_90 = per90_calculation(
            player.tackles_per_90, player_season.tackles_per_90
        )
        player.tackle_ratio = per90_calculation(
            player.tackle_ratio, player_season.tackle_ratio
        )
        player.cr_c += player_season.cr_c
        player.shot_percent = per90_calculation(
            player.shot_percent, player_season.shot_percent
        )
        player.header_percent = per90_calculation(
            player.header_percent, player_season.header_percent
        )
        player.int_per_90 = per90_calculation(
            player.int_per_90, player_season.int_per_90
        )
        player.dribbles_per_90 = per90_calculation(
            player.dribbles_per_90, 90 * player_season.dribbles / player_season.minutes
        )
        player.pass_completion = per90_calculation(
            player.pass_completion, player_season.pass_completion
        )
        player.key_passes_per_90 = per90_calculation(
            player.key_passes_per_90,
            90 * player_season.key_passes / player_season.minutes,
        )
        player.shots_per_90 = per90_calculation(
            player.shots_per_90, 90 * player_season.shots / player_season.minutes
        )
