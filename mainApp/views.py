...
class SaveView(LoginRequiredMixin, View):
	...
	def get(self, request, pk):
		...
		return render(request, 'mainApp/view_save.html',
					  {'save': save, 'players_json': players_json, 'season_json': season_json, 'save_no': save_no,
					   'season_positions': season_positions, 'best_players': json.dumps(best_players)})
...
class PlayerView(LoginRequiredMixin, View):
	...
	def get_indicator_95th():
		...
		data.update({'key_passes/90': Player.objects.order_by('-key_passes_per_90')[
			_95thpercentile].key_passes_per_90})
	...
	def get(self, request, pk, name):
		...
		return render(request, 'mainApp/view_player.html',
					  {'player': player, 'playerdata_json': playerdata_json, 'playerseasons': playerseasons,
					   'playingyears': years_played, 'seasons': seasons, 'percentile_data': percentile_data})
...
class SeasonView(LoginRequiredMixin, View):
	...
	def get(self, request, pk, pk2):
		...
		return render(request, 'mainApp/view_season.html',
					  {'season': season, 'players': players, 'playerseasons': playerseasons,
